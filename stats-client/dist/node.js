...
const wsUrl = process.env.NEXT_PUBLIC_WS_URL ||
    `ws://${process.env.HOST || 'localhost'}:${process.env.PORT || '3000'}${wsPath}`;
...
async function updateStats() {
    try {
        ...
        if (gethStats)
            stats.containers.geth = gethStats;
        if (beaconStats)
            stats.containers.beacon = beaconStats;
        if (validatorStats)
            stats.containers.validator = validatorStats;
        // Fetch beacon node stats
        const beaconNodeStats = await getBeaconNodeStats();
        if (beaconNodeStats)
            stats.beacon = beaconNodeStats;
    }
    catch (err) {
